...
#                 "color": "#1e88e5",
#                 "extendedProps": {
#                     "teacher": s.course_class.teacher.full_name,
#                     "course_code": s.course_class.course.course_code
#                 }
#             }
#             result.append(event)
...
def get_schedules(
    ...
    semester_id: Optional[str] = Query(None, description="Semester ID (if not provided, uses latest semester)"),
    db: Session = Depends(get_db)
):
    try:
        schedules = schedule_service.get_schedule_by_semester(db, program_id, semester_id)
...
def generate_schedule(
    ...
    try:
        result = schedule_service.generate_schedule(db, program_id)
