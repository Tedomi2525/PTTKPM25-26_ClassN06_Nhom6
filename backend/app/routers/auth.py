...
from app.schemas.auth import LoginRequest, Token, UserOut, UserPasswordUpdate, MessageResponse
...
def change_password(payload: UserPasswordUpdate, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    auth_service.update_user_password(db, current_user["user_id"], payload)
